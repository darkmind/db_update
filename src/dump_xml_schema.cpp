...
void populate_schema ( const shared_ptr<Node> schema, pt::ptree& xml_schema, const string& parent ) {
    string add_parent;
    if ( parent.empty() ) {
        add_parent = schema->get_name();
    }
    else {
        add_parent = parent + "." + schema->get_name();
    }

    if ( schema->has_children() ) {
        for ( auto child : schema->get_children() ) {
            populate_schema( child.second, xml_schema, add_parent );
        }
    }
    else {
        for ( auto data : schema->get_data() ) {
            xml_schema.add( add_parent + "." + data.first, data.second );
        }
    }
}
...
int main( int ac, char* av[] )
{
    ...
    const shared_ptr<Node> schema = core->get_schema_ref()->get_schema();

    pt::ptree xml_schema;
    populate_schema( schema, xml_schema, (string)"" );
